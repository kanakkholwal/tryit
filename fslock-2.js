                        ...
                        if (validurlit(url.value)) {
                          $.ajax({
                            ...
                            success: function(a){
                              ...
                              result.innerHTML = '<div class="alert alert-success text-center">'+config.lang.convertsuccess+'</div><div class="input-group  flex-nowrap mb-3"><span class="input-group-text"  data-clipboard-action='copy' data-clipboard-target='#cop1'  id="final-result"><i class="fas fa-copy fa-fw"></i></span><input type="text"  id ="copy1" class="form-control" onfocus="this.select()" onmouseup="return false"  value="'+linknya+'?u='+encodeURIComponent(CryptoJS.AES.encrypt(JSON.stringify(data), keyit))+'" aria-describedby="final-result"/></div>';
                              setTimeout(function(){
